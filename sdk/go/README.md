...
```bash
go get github.com/crafter-station/wapi/sdk/go@v0.3.1
```

Pin a tag for anything you deploy; `@main` moves. Note the tag Go resolves is `sdk/go/v0.3.1`,
not `v0.3.1` — a module in a subdirectory takes a tag prefixed with its path, and the repository
carries both so `@v0.3.1` above works as written.
